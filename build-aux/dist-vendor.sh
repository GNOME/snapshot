...
cargo vendor-filterer --all-features --platform=x86_64-unknown-linux-gnu --platform=aarch64-unknown-linux-gnu > "$MESON_PROJECT_DIST_ROOT"/.cargo/config.toml
...
sed -i 's/^directory = ".*"/directory = "vendor"/g' "$MESON_PROJECT_DIST_ROOT/.cargo/config.toml"
# Move vendor into dist tarball directory
mv vendor "$MESON_PROJECT_DIST_ROOT"
